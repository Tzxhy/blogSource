...
```
熟悉 react 的同学可能马上就有疑问了，这个 setInterval 不应该写在类似 useEffect 里，应该作为类似 onMount 执行？刚开始学习 solid 时，是会存在这些疑问的。这也是与 react 不同的地方之一。react中是 state 改变后，整个组件函数会重新执行一遍，返回一个新的 react 组件，在组件中的代码当然会重新执行一遍；而 solid 中， state 改变后，仅会更新依赖该 state 的 **子组件块**，不会走整个组件重新执行的逻辑，因此直接在组件体写 setInterval 是没有问题的。

在 soild 中访问 state，是需要执行 createSignal 返回的第一项索引的，看看其签名：
```ts
...
```
返回的 getter 是一个类似于对象的 get 属性。为什么要设计成 get 方法，而不是像 react 的直接变量引用呢？这是 solid 的设计如此，这样才能 **捕获依赖**。那么又有人要问了，为什么不做成 vue2 中的对象属性引用呢？为什么不做成 vue3 中的 ref/reactive 呢？哪有这么多为什么，只能说 solid 设计如此。每一种响应式的基本原理都一样，但实现上可以是千差万别的。

### 捕获依赖
关于响应式原理，之前有写过一篇文章及对应的库，有兴趣的可以看看[文章](https://reactivity.tanzhixuan.site/)，同时代码也放在了 github 上。这里简单说说。

何为捕获依赖？当我们在组件的某处依赖了某个 state 时，程序如何感知更新了 state 后触发对应的节点内容更新呢？其实这就是捕获依赖：在依赖了该 state 的方法执行时，记录下执行该方法的引用。比如：
```tsx
function Counter() {
  const [count, setCount] = createSignal(0);
...
```
所以，程序能记录下，当 count 执行时，其实是需要重新执行： _$insert 这里相关的代码来更新 UI。这就是捕获依赖的大致理解。

捕获依赖，一般是基于 Object.defineProperty 或者是基于 Proxy 来做，同样可以看看上面贴的文档链接。这里不详细说响应式原理。
...
```
因为 doubleCount 中依赖了 count，当 count 更新时，会触发 doubleCount 重新执行， doubleCount 更新后，又会触发 div 下的更新。

### Effect
solid 中同样有 类似 useEffect 的api： createEffect。
```tsx
function Counter() {
  const [count, setCount] = createSignal(0);
  createEffect(() => {
...
```
createEffect 是一个显式的捕获依赖：当内部依赖的 state 更新后，会触发该 effect 的重新执行。
...
```
仅当 count 变化时，才会触发 fib 函数的重新执行。在上例中，点击按钮后，虽然有那么多个 fib() 的执行，但最终，其实只有一次执行。memo 与常见的 useMemo 等 api 一致，仅当依赖更新后，才重新触发执行。这里说的是更新，比如，依赖值从0 => 1，会导致重新执行一次；从1 => 0，也会导致重新执行，并没有一个值缓存的作用。
...
```
`Show` 组件，接收一个 when 参数和 fallback 参数，用于控制显示哪个。
...
```
solid 中 For 的参数是数据项和索引 Signal，具体差异可以看官网例子。For 的item 渲染是根据每一个item引用的变化来的：仅当某个 item 引用发生改变时，才会重新渲染该 item 项，而其他未发生改变的item，则复用原来的组件。
...
### 样式
Solid 通过调用 style.setProperty 的封装来进行样式设置。这意味着键需要采用破折号的形式，如 background-color 而不是 backgroundColor。
```tsx
function App() {
...
```
第一眼看来，App 中的 ref canvas 是反直觉的：自组件中没有一种魔法可以修改 canvas 的引用。按照我们的理解，如果没有某种魔法，canvas 变量要么是 undefined，要么是 Canvas 示例，但该演示中， canvas 变量却指向 canvas 画布元素。我们来看下编译后的代码：
```js
function App() {
  let canvas;
  onMount(() => {
...
### 指令
Solid 通过 use: 命名空间支持自定义指令。但这只是 ref 一个有用的语法糖，类似于原生的绑定，并且可以在同一个元素上有多个绑定而不会发生冲突。solid 中的指令有点 vue 的那味：
```tsx
...
```
react 版本下，要更新某一个 item 的值，就需要更新 state，而更新 state，就会生成新的组件，然后走 diff, patch 的流程。
...
```
在 vue2 版本中，点击按钮来更新第一项的name属性，原意也仅是更新第一项，但实际上同样会执行整个 template 的重新渲染，因此未被改变的第二项，同样会被渲染出来。
...
```
可能又有人要说了，为什么可以在 handler 里使用 createSignal ？react 中的 useXXX 都必须是在顶级呀！没错，这是差异点：react 中需要根据调用顺序来确认相关的 state 和 effect，但 solid 中，这些仅仅被称为 **响应式**，它只是说明该引用是响应式的。

这个新版本的细粒度的响应性方案中，更新某个 item 的某个字段值，不会导致列表的整体更新，也不会导致依赖该 item 的DOM 的替换更新，仅仅是更新了该DOM依赖该字段值的子DOM。
...
```
利用 Store，可以实现类似 Vue 中深层监听的功能。但又有不同：vue 中某个对象的键值改变，会导致整个template重新渲染，而 solid 只更新依赖该 item 的 该 key 的子DOM部分。
